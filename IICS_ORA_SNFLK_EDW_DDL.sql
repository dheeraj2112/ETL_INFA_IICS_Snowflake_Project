--Creating the database and schemas as required by IICS mappings.
...
--3. Create the schemas, as per requirements

CREATE SCHEMA IF NOT EXISTS EDW_STG;
CREATE SCHEMA IF NOT EXISTS EDW;
CREATE SCHEMA IF NOT EXISTS EDW_EXTR;
CREATE SCHEMA IF NOT EXISTS EDW_TEST;

---STG Table(s) creation 

---EDW_STG TABLE DEFINITION
-- Use context
--Mapping will process the STG Load as SCD Type-1 from source.
...
--Use contexts
--Additional Columns as needed for the SCD Type-2 requirements (both end data and active flag methods are used)
--Additonal surrogate keys in the EDW tables 
...
CREATE OR REPLACE TABLE EDW.EDW.REGIONS  
    ( 
	  REGION_KEY INT AUTOINCREMENT START 1 INCREMENT 1 ORDER
	, REGION_ID      NUMBER   
    , REGION_NAME    VARCHAR2(25)
    , MD5_HASH  VARCHAR2(32)
	, EDW_UPDATED_BY VARCHAR2(32)
...
CREATE OR REPLACE TABLE EDW.EDW.COUNTRIES   
    ( 
	  COUNTRY_KEY  INT AUTOINCREMENT START 1 INCREMENT 1 ORDER
	, COUNTRY_ID      CHAR(2)   
    , COUNTRY_NAME    VARCHAR2(40)   
    , REGION_ID       NUMBER    
    , MD5_HASH  VARCHAR2(32)
...
CREATE OR REPLACE TABLE EDW.EDW.LOCATIONS  
    ( 
	  LOCATION_KEY INT AUTOINCREMENT START 1 INCREMENT 1 ORDER
	, LOCATION_ID    NUMBER(4)  NOT NULL 
    , STREET_ADDRESS VARCHAR2(40)  
    , POSTAL_CODE    VARCHAR2(12)  
    , CITY           VARCHAR2(30)  
...
CREATE OR REPLACE TABLE EDW.EDW.DEPARTMENTS  
    ( 
	  DEPARTMENT_KEY INT AUTOINCREMENT START 1 INCREMENT 1 ORDER
	, DEPARTMENT_ID    NUMBER(4) 
    , DEPARTMENT_NAME  VARCHAR2(30)  
    , MANAGER_ID       NUMBER(6)  
    , LOCATION_ID      NUMBER(4)
...
CREATE OR REPLACE TABLE EDW.EDW.JOBS  
    ( 
	  JOB_KEY INT AUTOINCREMENT START 1 INCREMENT 1 ORDER
	, JOB_ID         VARCHAR2(10)  
    , JOB_TITLE      VARCHAR2(35)  
    , MIN_SALARY     NUMBER(6)  
    , MAX_SALARY     NUMBER(6) 
...
CREATE OR REPLACE TABLE EDW.EDW.EMPLOYEES  
    ( 
	  EMPLOYEE_KEY INT AUTOINCREMENT START 1 INCREMENT 1 ORDER
	, EMPLOYEE_ID    NUMBER(6)
    , FIRST_NAME     VARCHAR2(20)  
    , LAST_NAME      VARCHAR2(25)   
    , EMAIL          VARCHAR2(25)  
...
CREATE OR REPLACE TABLE EDW.EDW.JOB_HISTORY  
    ( 
	  JOB_HISTORY_KEY INT AUTOINCREMENT START 1 INCREMENT 1 ORDER
	, EMPLOYEE_ID   NUMBER(6)  
    , START_DATE    DATE  
    , END_DATE      DATE  
    , JOB_ID        VARCHAR2(10)  
